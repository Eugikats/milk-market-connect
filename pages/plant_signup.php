...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $processing_capacity = floatval($_POST['processing_capacity'] ?? 0);
    
    // Validate input
    if (empty($name) || empty($email) || empty($password) || empty($location) || empty($contact) || $processing_capacity <= 0) {
        $error = 'All fields are required and must be valid.';
    } else {
        // Check if email already exists
        $checkEmail = mysqli_query($conn, "SELECT id FROM processing_plants WHERE email = '$email'");
        if (mysqli_num_rows($checkEmail) > 0) {
            $error = 'Email already exists. Please use a different email or login.';
        } else {
            // Insert new processing plant
            $sql = "INSERT INTO processing_plants (name, email, password, location, contact, processing_capacity) 
                    VALUES ('$name', '$email', '$password', '$location', '$contact', $processing_capacity)";
...
<head>
    ...
    <title>Processing Plant Sign Up - Milk Market Connect</title>
...
<body>
    ...
    <div class="auth-container">
        ...
        <h1>Processing Plant Sign Up</h1>
        ...
        <form method="post" class="auth-form">
            ...
            <div class="form-group">
                <!-- Label: Displayed as block, margin below, medium font weight -->
                <label for="processing_capacity">Processing Capacity (liters per day)</label>
                <!-- Input field: Full width, padding, border, rounded corners, specific font size, focus style -->
                <input type="number" id="processing_capacity" name="processing_capacity" min="1" step="0.1" required value="<?php echo $_POST['processing_capacity'] ?? ''; ?>">
            </div>
